...
class TestImageReceivers:
    ...
    def test_compress_category_image(self, mocker, model, receiver, image_format):
    ...
    def test_delete_category_image(self, mocker, model, receiver):
...
class TestStatisticsReceivers:
    def test_handle_category_viewed(self, mocker):
        # Mock the `update_category_views` task
        mock_task = mocker.patch.object(update_category_views, "apply_async")

        # Call the receiver
        handle_category_viewed(sender=CategoryStatisticMiddleware, category_id=1)

        # Check that the task has been called once with the correct arguments
        mock_task.assert_called_once_with(args=(1,), queue="low_priority", priority=10)

    def test_handle_category_purchase(self, mocker):
        # Mock the `update_category_purchase` task
        mock_task = mocker.patch.object(update_category_purchase, "apply_async")
