...
class TestModelCategory:
    ...
    def test_absolute_url(self):
        ...
        assert absolute_url == settings.BASE_FRONTEND_URL + category.url

    @pytest.mark.parametrize("image_attr", (True, False))
    def test_category_image(self, mocker, image_attr):
        # Mock an image instance
        mock_image = mocker.MagicMock(spec=CategoryImage)
        mock_image._state = mocker.MagicMock()

        # Create category instance
        category = Category(image=mock_image) if image_attr else Category()
    ...
    def test_category_card(self, mocker, card_attr):
        ...
        # Create category instance
        category = Category(card=mock_card) if card_attr else Category()
    ...
    def test_category_statistics(self, mocker, statistics_attr):
        # Mock a statistics instance
        mock_statistics = mocker.MagicMock(spec=Statistics)
        mock_statistics._state = mocker.MagicMock()

        # Create category instance
        category = Category(statistics=mock_statistics) if statistics_attr else Category()
    ...
    def test_clean(self, mocker):
        ...
        # Check that method `clean_url` called once
        mock_clean_url.assert_called_once()
    ...
    def test_generate_url_with_children(self, mocker):
    ...
    def test_mptt_methods(self, mocker, method):
