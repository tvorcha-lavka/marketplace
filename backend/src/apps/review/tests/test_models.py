...
class TestReview:
    ...
    def test_calculate(self, mocker: MockerFixture) -> None:
...
class TestUserReview:
    @pytest.fixture(autouse=True)
    def setup(self) -> None:
        self.user = User(pk=uuid4(), username="test_user")
    ...
    def test_str_method(self) -> None:
        expected_result = (
            ...
            f"Review: '{self.instance.message[:20] if self.instance.message else ''}...'"
        )

        # Assert the expected behavior
        assert str(self.instance) == expected_result

    def test_update(self, mocker: MockerFixture) -> None:
        # Mock `save` method
        mocker.patch.object(self.instance, "save", return_value=None)
