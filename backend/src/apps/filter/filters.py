...
class FilterTypeFilterAdmin(SimpleListFilter):
    title = _("filter type")
    ...
    def lookups(
        self,
        request: HttpRequest,
        model_admin: ModelAdmin[FilterType],
    ) -> Iterable[tuple[str, "_StrOrPromise"]] | None:
        qs: QuerySet[FilterType] = FilterType.objects.all()

        if filter_id := self.value():
            qs = qs.filter(id=int(filter_id))

        queryset = qs.prefetch_related("translations")
        data = [(str(obj.pk), obj.filter_name) for obj in queryset]
    ...
    def queryset(
        self,
        request: HttpRequest,
        queryset: QuerySet[FilterType],
    ) -> QuerySet[FilterType] | None:

        if filter_id := self.value():
            return queryset.filter(filter_type_id=int(filter_id))  # type: ignore[misc]
...
class FilterTypeFilter(FilterSet):
    ...
    @staticmethod
    def filter_by_category_id(queryset: QuerySet[FilterType], _name: str, value: int) -> QuerySet[FilterType]:
